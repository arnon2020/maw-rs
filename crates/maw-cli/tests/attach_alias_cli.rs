...
fn attach_text_and_readonly_recovery_match_maw_js_surface() {
    ...
    assert!(!missing
        .stdout
        .contains("maw wake unlikely-attach-missing-363 --attach"));
}

#[test]
fn attach_usage_and_default_menu_show_curated_core_commands() {
    let usage = run(&[]);
    assert_eq!(usage.code, 0);
    assert!(usage.stdout.contains("core commands:"));
...
fn attach_alive_equals_and_text_recover_paths_are_covered() {
    ...
    assert!(recover_text
        .stdout
        .contains("attach: 'unlikely-attach-text-missing-363' not found"));
    assert!(!recover_text
        .stdout
        .contains("maw wake unlikely-attach-text-missing-363 --attach"));
}
...
fn attach_without_fake_alive_uses_live_probe_recover_path() {
    ...
    assert!(missing
        .stdout
        .contains("attach: 'unlikely-goal-coverage-session-zz-1850' not found"));
}
